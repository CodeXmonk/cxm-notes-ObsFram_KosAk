...
&nbsp;
Ugly and dirty sketch inspired by:
* [BMI Calculator](https://beta.observablehq.com/@jashkenas/bmi-calculator) by [Jeremy Ashkenas @jashkenas](https://beta.observablehq.com/@jashkenas).
* An idea I've found in Larry Hoover's book - [Metabolic States: Notes on Stress, Nutrition and Exercise](https://play.google.com/store/books/details/Larry_Hoover_Metabolic_States?id=aS4SAgAAQBAJ). There's a companion volume to the book - [The Metabolic States Workbook: Track and Record Changes in Bodyweight and Composition](https://play.google.com/store/books/details/Larry_Hoover_The_Metabolic_States_Workbook?id=4C5CDwAAQBAJ).

After reading [3 key insights that make D3.js easy to learn](https://swizec.com/blog/3-key-insights-make-d3-js-easy-learn/swizec/8179) by Swizec Teller I start rethink the whole concept - starting principally with data structure.

Right now **ONLY METRIC** - as I'm not familiar width imperial system..:-(

```js
//const startview = Inputs.date({label: "Kezdő dátum", min:"2003-01-01", max:"2024-12-31", value: cxm_start})
//const start = view(startview)

//viewof system = radio({options: ["imperial", "metric"], value: "metric"})
//const system = view(Inputs.radio(["imperial", "metric"], {label: "mérték"}));
const system = "metric";
```

```js
//viewof gender = radio({options: ["male", "female"], value: "male"})
...
function MetComchart() {
  ...
  const svg = DOM.svg(w + margin.left + margin.right, h + margin.top + margin.bottom);
  ...
  const legend_Values_Vertical_rects = [["2","thin light"],
  ...
  const legend_Values_Vertical_lines = [["TV_line","TV: Total Body Volume"],
  ...
  const legend_Values_CatAna = [["1","7","Bodyfat gain","LBM catabolism, FAT anabolism"],
  ...
  const legend_Values_Sheldon = [["7","6","ENDOMORPHIC","high bodyweight"],
                               ["3","5","MESOMORPHIC","low bodyfat percentage"],
                               ["1","2","ECTOMORPHIC","low bodyweight"]];
  //(dx,dy,rectHeight,rectWidth,dy_rect,dx_rects,TITLE,legend_Values,dx_round_brackets)
  ...
  function grid_lines() {
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  const startArray_weight = Array.apply(null, {length: weight_MAX }).map(Number.call, Number).slice(weight_MIN);
  const evenArray_weight = evenNumbers(startArray_weight);
  //const evenArray_BMI = evenArray;
  //const evenArray_BMI = evenArray_BMI.map(x => x / (height*height));
    /*
    evenArray_BMI.forEach((num, index) => {
    return evenArray_BMI[index] = num / (height*height);
});
    */
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    g.append('rect')
  ...
  g.append('g')
    ...
    .call(d3.axisTop(x)
      //.tickSizeInner(h + 6).tickSizeOuter(0).ticks(10)
      .tickSizeInner(6).tickSizeOuter(0).ticks(10)
      .tickValues( evenArray_weight )
      .tickFormat(function(d, i){ return parseFloat(d / Math.pow((height/100),2)).toFixed(1); }))
      //.tickFormat(function(d, i){ return d / 2; }))
      .append("text")
        .attr("fill", "#000")
        .attr("x", w)
        .attr("y", -8)
      ...
      .style("font-weight", "bold")
        .text("BMI");
  evenArray_weight.splice(-1,1)
  g.append('g')
    .call(d3.axisBottom(x)
      //.tickSizeInner(h + 6).tickSizeOuter(0).ticks(10)
      .tickSizeInner(h + 6).tickSizeOuter(0).ticks(10).tickValues( evenArray_weight ))
      .append("text")
        .attr("fill", "#000")
        .attr("x", w)
        .attr("y", h + 16)
    ...
    //g.selectAll(".tick:not(:first-of-type) line").attr("stroke", "#c0c0c0");
    g.selectAll(".tick:not(:first-of-type) line").classed("minor", true);
    g.append('g')
      .attr("class", "grid")
      .call(d3.axisBottom(x)
        .tickSizeInner(h).tickSizeOuter(0).ticks(40));
    g.selectAll(".tick:not(:first-of-type) line").attr("stroke", "#c0c0c0");
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  const startArray_fat = Array.apply(null, {length: fat_MAX - 1}).map(Number.call, Number).slice(fat_MIN);
  const evenArray_fat = evenNumbers(startArray_fat);
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  g.append('g')
    .call(d3.axisLeft(y)
          //.tickSize(-w)
      .tickSizeInner(6).tickSizeOuter(0).ticks(10).tickValues( evenArray_fat ))
      //.tickSizeInner(6).tickSizeOuter(0).ticks(10))
    .append("text")
      .attr("fill", "#000")
    ...
    g.append('g')
      .attr("class", "grid")
      .call(d3.axisLeft(y).tickSize(-w).ticks(20))
    g.selectAll(".tick:not(:first-of-type) line").attr("stroke", "#c0c0c0");
  }
///////////////////////////////////////////////////////////////////////////////////////
/*using Devine equation*/
function get_body_mass_ideal(gender) {
  const body_mass_ideal;
  if(gender=="male"){
...
///////////////////////////////////////////////////////////////////////////////////////
  ...
  function area_4() {
    ...
    fat_ideal_returned = get_fat_ideal_by_age_and_gender(Age);
  
    const poly = [{"x":get_weight_average_min(gender), "y":fat_ideal_returned["min"]},//fat_ideal.min},
  ...
  function area_8() {
    ...
    fat_average_returned = get_fat_average_by_age_and_gender(Age);
    const poly = [{"x":get_weight_average_min(gender), "y":fat_average_returned["min"]},//fat_ideal.min},
  ...
  function draw_legend_Vertical_lines(dx,dy,lineLength, dy_lines, dx_text,legend_Values) {
    /*
    const legend_Values = [["TV_line","TV: Total Body Volume"],
                        ["BF_line","BF: Bodyfat curve"],
                        ["LBM_line","LBM curve"]]
    */
  ...
  function draw_legend_Bottom_Horizontal(dx,dy,rectHeight,rectWidth,dy_rect,dx_rects,title,legend_Values,dx_round_brackets) {
    /*
    const legend_Values = [["1","7","Bodyfat gain","LBM catabolism, FAT anabolism"],
                         ["3","5","Muscle gain","LBM anabolism, FAT catabolism"],
                         [0,"2","Weight loss","LBM & FAT catabolism"],
                         [0,"6","Weight gain","LBM & FAT anabolism"]]
    */
  ...
  //draw_legend_Bottom_Horizontal END /////////////////////////////////////////////////////////////////////// 
  return g.node();
}
```
...
//weight_average_min = weight_average_min.toFixed(2);
```
...
```js
weight_MIN = Math.round(get_body_mass_ideal(gender) - 15)
```

```js
weight_MAX = Math.round(get_body_mass_ideal(gender) + 15)
```

```js
fat_MIN = Math.round(get_fat_perc_average(Age)-11)
```

```js
fat_MAX = Math.round(get_fat_perc_average(Age)+11)
```
...
```js
const data_adults_weight_to_height_ratio = [["4'6\"",  "137", "63-77", "28.5-34.9", "63-77", "28.5-34.9"],
